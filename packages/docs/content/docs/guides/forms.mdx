...
Forms in Acronis UIKit are built from composable primitives. The library ships two layers:

- **UI layer** — `Input`, `Textarea`, `Select`, `Checkbox`, `Switch`, `RadioGroup`, `Combobox`, `DatePicker`, `NumberField`, `Label` — the visual building blocks.
- **Form layer** — `FormItem`, `FormLabel`, `FormControl`, `FormDescription`, `FormMessage` — layout and accessibility wiring that connects controls to form state.

The form layer is UI-framework-agnostic by design. The `Form` component (and `FormField`) provide a React Hook Form integration, but the lower-level primitives work equally well with TanStack Form or any other state manager.
...
<DemoPreview sourcePath="packages/demos/src/form/FormLayoutTwoColumn.tsx">
  <FormLayoutTwoColumn />
</DemoPreview>
...
<DemoPreview sourcePath="packages/demos/src/form/FormLayoutSections.tsx">
  <FormLayoutSections />
</DemoPreview>
...
<DemoPreview sourcePath="packages/demos/src/form/FormTanstackBasic.tsx">
  <FormTanstackBasic />
</DemoPreview>

### Multiple fields with select

<DemoPreview sourcePath="packages/demos/src/form/FormTanstackProfile.tsx">
  <FormTanstackProfile />
</DemoPreview>
...
export function TanstackLoginForm() {
  ...
  return (
    ...
    >
      ...
      {/* Subscribe to only the fields you need */}
...
- **No `FormProvider`** — TanStack Form uses its own context via `form.Field` render-prop children. Use `FormItem`, `FormLabel`, `FormControl`, `FormDescription`, and `FormMessage` as standalone layout primitives (they work without `Form`/`FormField` context).
...
- **`htmlFor` must be manual** — since there is no `FormField` context, pass `id={field.name}` on the control and `htmlFor={field.name}` on `FormLabel`.
- **Error display is explicit** — check `field.state.meta.errors` and render `<FormMessage>` conditionally.
- **Touched state** — validate-on-blur by default; use `field.state.meta.isTouched` to defer error display until after the user has left the field.
